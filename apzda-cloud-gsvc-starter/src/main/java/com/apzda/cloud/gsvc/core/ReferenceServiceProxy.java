...
public class ReferenceServiceProxy implements InvocationHandler {
    ...
    public ReferenceServiceProxy(ReferenceServiceFactoryBean serviceProxyFactoryBean) {
        ...
        serviceName = serviceProxyFactoryBean.getServiceName();
        appName = serviceProxyFactoryBean.getAppName();
        String contextPath = serviceProxyFactoryBean.getContextPath();
        serviceIndex = serviceProxyFactoryBean.getServiceIndex();

        objectMapper = ResponseUtils.OBJECT_MAPPER;

        svcConfigure = applicationContext.getBean(GatewayServiceConfigure.class);
        ServiceConfigurationProperties.ServiceConfig serviceConfig = svcConfigure
            .getServiceConfig(serviceProxyFactoryBean.getServiceIndex());

        log.info("For Service {}@{} Will try picking an instance via load-balancing: http://{}", serviceName, appName,
                appName);

        url = generateUrl(appName, contextPath);
    ...
    private Mono<String> prepareRequestBody(WebClient.RequestBodySpec req, Object requestObj) {
        ...
        if (StpUtil.isLogin()) {
            ...
            log.trace("[{}] Set Sa-Token header: '{}: {}'", requestId, tokenInfo.tokenName, tokenInfo.tokenValue);
    ...
    public String toString() {
        return new ToStringCreator(this).append("app", appName)
    ...
    private static String generateUrl(String appName, String contextPath) {
        if (StringUtils.isNotBlank(contextPath)) {
            contextPath = StringUtils.strip(contextPath, "/ ");
            if (StringUtils.isNotBlank(contextPath)) {
                return "http://" + appName + "/" + contextPath;
            }
        }
        return "http://" + appName + "/" + appName;
    }
